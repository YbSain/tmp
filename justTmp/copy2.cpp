...
int main(void) {
    ...
    while (true) {
        ...
        Mat displaymorph = makegray(frame);
        Labeling(displaymorph);
...
void Labeling(Mat displaymorph){
    ...
    static Point2d firstCenter(displaymorph.cols / 2, displaymorph.rows / 2);
    int cnt = connectedComponentsWithStats(displaymorph, labels, stats, centroids);
        for (int i = 1; i < cnt; i++) {
            ...
            int error = displaymorph.cols / 2 - (int)c[0];
